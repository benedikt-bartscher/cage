...
update_capabilities(struct cg_seat *seat)
{
	...
	/* Senvend kiosk fork: cursor hidden by default (touch-only deployments
	 * would otherwise flash a cursor between cage taking DRM master and the
	 * first wayland client connecting to provide a cursor surface; the
	 * eGalax-class touchscreen reports POINTER capability so upstream's
	 * "if (no pointer) unset; else set xcursor" always lands on set).
	 * `cage -c` restores upstream behaviour for debugging / non-kiosk runs.
	 */
	if (seat->server->show_cursor && (caps & WL_SEAT_CAPABILITY_POINTER) != 0) {
